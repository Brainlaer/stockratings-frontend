...
<template>
    <div class="card p-2">
        <MegaMenu :model="items" class="p-4 bg-surface-0" style="border-radius: 3rem">
            <template #start>
                <i class="pi pi-building-columns pl-3" style="font-size: 1.5rem"></i>
            </template>
            <template #item="{ item }">
                ...
                <div v-else class="flex flex-col items-start gap-4 p-2">
                    ...
                    <Button :label="String(item.label)" outlined />
                </div>
            </template>
            <template #end>
                <Button icon="pi pi-eye" label="Admin" rounded @click="goTo('stockRatingList')"/>
            </template>
        </MegaMenu>
    </div>
</template>
...
import { ref } from "vue";
import { MegaMenu, Button } from "primevue";
